...
export async function readProjects(options = {}) {
  ...
  try {
    ...
    return (Array.isArray(rows) ? rows : []).map((row) => mapDbProject(row)).filter((project) => project.owner === "ali");
  } catch (error) {
    ...
    return (await readLocalProjects()).map((row) => normalizeProject(row, row.id)).filter((project) => project.owner === "ali");
  }
}
