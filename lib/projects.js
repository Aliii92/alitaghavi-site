...
export async function upsertSingleProject(project) {
  ...
  await supabaseUpsert(projectsTable, [mapProjectToDb(normalized)], "id");
  ...
  return mapDbProject({
    ...saved,
    subArea: saved.subArea ?? saved.sub_area,
    paymentPlan: saved.paymentPlan ?? saved.payment_plan,
    handoverDate: saved.handoverDate ?? saved.handover_date,
    startingPrice: saved.startingPrice ?? saved.starting_price,
    whatsappLink: saved.whatsappLink ?? saved.whatsapp_link,
    image: saved.image ?? saved.image_url ?? saved.cover_image ?? saved.thumbnail
  });
}
