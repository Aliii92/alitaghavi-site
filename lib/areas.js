...
async function readLocalAreas() {
  try {
    ...
    return normalized.filter((area) => area.owner === "ali");
  } catch {
...
export async function readAreas(options = {}) {
  ...
  try {
    ...
    return normalized.filter((area) => area.owner === "ali").length ? normalized.filter((area) => area.owner === "ali") : ownerScopedDefaults();
  } catch (error) {
