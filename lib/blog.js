...
function normalizeBlogPost(record = {}) {
  ...
  return {
    id: safeString(record.id || slug || titleEn.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "")),
    title_en: titleEn,
...
function sanitizeBlogPayload(record = {}) {
...
export async function upsertSingleBlogPost(record) {
  const payload = sanitizeBlogPayload(record);
  console.log("[blog] upsert payload keys:", Object.keys(payload));
...
export { BLOG_PLACEHOLDER, BLOG_TABLE, normalizeBlogPost, sanitizeBlogPayload };
