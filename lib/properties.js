...
import { normalizeDeal, sortPropertiesByDeals } from "./property-deals.js";
...
export function mapPropertyToDb(property, table = standardTable) {
  const basePayload = {
    ...
    deal: normalizeDeal(property.deal),
...
async function readPropertyRecords(options = {}) {
  ...
  try {
    if (inventoryType === "resale-off-plan") {
      ...
      return (Array.isArray(resaleRows) ? resaleRows : [])
        .map((row) => mapDbProperty(row, resaleTable))
        .filter((property) => property.owner === "ali");
    }
    ...
    if (inventoryType === "ready" || inventoryType === "off-plan" || inventoryType === "standard") {
      ...
      return (Array.isArray(standardRows) ? standardRows : [])
        .map((row) => mapDbProperty(row, standardTable))
        .filter((property) => property.owner === "ali");
    }
    ...
    return (await readLocalProperties()).filter((property) => normalizeProperty(property, property.id).owner === "ali");
  }
}

// Public consumers never receive private notes or unavailable records.
export async function readProperties(options = {}) {
  const properties = await readPropertyRecords(options);
  if (options.allowFallback === false) return properties;
  return sortPropertiesByDeals(properties.filter(isPubliclyVisibleProperty)).map(({ notes, ...property }) => property);
}
...
export function normalizeProperty(input, fallbackId = "") {
  ...
  const normalized = {
    ...
    owner: "ali",
    title: String(input.id || fallbackId).startsWith("dl-") && input.building ? [input.building, input.bedrooms].filter(Boolean).join(" · ") : input.title || "",
    area: input.area || "",
    region: input.region || "",
    ...
    deal: normalizeDeal(input.deal),
    gallery_images: Array.isArray(input.gallery_images) ? input.gallery_images.filter(url => typeof url === "string" && /^https?:\/\//.test(url)).slice(0, 30) : [],
    floor_plan_url: /^https?:\/\//.test(input.floor_plan_url || "") ? input.floor_plan_url : "",
    plot_size: String(input.plot_size || "").slice(0,80),
    video_url: /^https:\/\//.test(input.video_url || "") ? input.video_url : "",
