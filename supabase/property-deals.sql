    ...
    if not (r->>'publish')::boolean then
      ...
      merged := coalesce(old_row, jsonb_build_object('owner','ali','featured',false,'gallery_images','[]'::jsonb,'created_at',now())) ||
        jsonb_build_object('id',m.property_id,'area',r->>'area','building',r->>'building',
          ...
          'deal',coalesce(r->'deal',old_row->'deal','{}'::jsonb),'project',r->>'building','handover',r->>'handover','status',r->>'status','updated_at',now());
      ...
      if old_table = target_table then
        ...
        execute format('update public.%I set area=$2->>''area'', building=$2->>''building'', property_type=$2->>''property_type'', bedrooms=$2->>''bedrooms'', size=$2->>''size'', price=$2->>''price'', view=$2->>''view'', status=$2->>''status'', deal=$2->''deal'', updated_at=now() where id=$1',target_table)
