...
const content = {
  en: {
    ...
    hero: {
      ...
      titleLines: ["Your next chapter.", "A remarkable Dubai address."],
  ...
  fa: {
    ...
    hero: {
      ...
      titleLines: ["خانه‌ای برای زندگی،", "انتخابی برای آینده."],
      text: "با علی تقوی، املاک آماده و پروژه‌های منتخب دبی را بر اساس بودجه، سبک زندگی و هدف سرمایه‌گذاری خود بررسی کنید.",
      whatsapp: "واتساپ",
...
export default function HomePage({ initialLocale = "en" }) {
  ...
  useEffect(() => {
    setLocale(window.location.pathname.startsWith("/fa") ? "fa" : "en");
  }, [pathname]);
  ...
  async function handleSubmit(event) {
    ...
    try {
      const response = await fetch("/api/leads", {
        ...
        body: JSON.stringify({ ...formData, source_page: "Consultation form", advisor_name: "Ali Taghavi",
          language_mode: locale.toUpperCase(), referrer_url: window.location.href,
          message_preview: formData.message, lead_type: "consultation" })
      });
  ...
  return (
    <ProfileHomePage
      ...
      search={{
        ...
        title: locale === "fa" ? "ملک مناسب خود را در دبی پیدا کنید" : "Find the right Dubai opportunity",
        text: locale === "fa" ? "بر اساس منطقه، ساختمان، تعداد خواب، بودجه و وضعیت ملک جست‌وجو کنید." : "Search by area, building, bedrooms, price, and ready or off-plan status.",
        filtersProps: {
          mode: "redirect",
          areaName: "Dubai",
          ...
          intro: locale === "fa" ? "املاک آماده، آف‌پلن و فروش مجدد را جست‌وجو کنید." : "Search across curated ready and off-plan opportunities, then continue to the full listings page."
        }
      }}
      ...
      featured={{
        ...
        moreLabel: t.featured.moreOptions || (locale === "fa" ? "مشاهده فرصت‌های بیشتر" : "Explore More Opportunities →")
      }}
      projects={{
      ...
      contact={{
        ...
        submitState,
        followUpHref: buildWhatsAppUrl(formData, locale),
        ...
        youtube: t.contact.youtube || (locale === "fa" ? "تماشا در یوتیوب" : "Watch on YouTube"),
